...
for (rtr_name,vlan) in rtr_names:
    ...
    #print tf
    tf.save_object_to_file("tf_simple_stanford_backbone/%s.tf"%rtr_name)
    id += 1
    cs_list[rtr_name] = cs
    
f = open("tf_simple_stanford_backbone/port_map.txt",'w')
...
for (from_router,from_port,to_router,to_port) in topology:
    ...
    tf.add_link_rule(rule)
tf.save_object_to_file("tf_simple_stanford_backbone/backbone_topology.tf")
